  ...
  def create
    ...
    respond_to do |format|
      if @product_image.save
        format.html { redirect_to edit_supplier_account_product_path(current_supplier, @product), :notice => "#{t('activerecord.successful.messages.created', :model => @product_image.class.model_name.human)}" }
      else
        @supplier_account = current_supplier.supplier_account
    		@product = Product.find params[:product_id]
  ...
  def update
    ...
    respond_to do |format|
      if @product_image.update_attributes(params[:product_image])
        format.html { redirect_to edit_supplier_account_product_path(current_supplier, @product), :notice => "#{t('activerecord.successful.messages.updated', :model => @product_image.class.model_name.human)}" }
        format.json { head :ok }
      else
     		@supplier_account = @supplier.supplier_account
		    @product = Product.find params[:product_id]
