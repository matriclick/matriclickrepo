  ...
  def create
    ...
    respond_to do |format|
      if @product_type.save
        format.html { redirect_to @product_type, :notice => "#{t('activerecord.successful.messages.created', :model =>  @product_type.class.model_name.human)}" }
  ...
  def update
    ...
    respond_to do |format|
      if @product_type.update_attributes(params[:product_type])
        format.html { redirect_to @product_type, :notice => "#{t('activerecord.successful.messages.updated', :model =>  @product_type.class.model_name.human)}" }
        format.json { head :ok }
      else
