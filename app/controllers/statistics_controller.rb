class StatisticsController < ApplicationController
  ...
  before_filter :authenticate_supplier!, :set_supplier_layout, :find_supplier, :authenticate_analytics
  ...
  def index
    ...
    # end
		...
		@total_views = get_monthly_views(supplier_account)
    #Get Messages params
    #@product_conversations = get_last_conversations(current_supplier.supplier_account.conversations.of_products, 6)
    #@service_conversations = get_last_conversations(current_supplier.supplier_account.conversations.of_services, 6)
...
private
  ...
  def get_monthly_views(supplier_account, time_span = 6)
    ...
    # end
		...
		supplier_account.supplier_page_views.where('created_at > ?', Date.today - time_span.months).order('created_at ASC').collect do |x|
