  ...
  def create
    ...
    respond_to do |format|
      if @service_image.save
        format.html { redirect_to edit_supplier_account_service_path(current_supplier, @service), :notice => "#{t('activerecord.successful.messages.created', :model =>  @service_image.class.model_name.human)}"  }
      else
		    @supplier_account = current_supplier.supplier_account
    		@service = Service.find params[:service_id]
  ...
  def update
    ...
    respond_to do |format|
      if @service_image.update_attributes(params[:service_image])
                format.html { redirect_to edit_supplier_account_service_path(current_supplier, @service), :notice => "#{t('activerecord.successful.messages.updated', :model =>  @service_image.class.model_name.human)}"  }
        format.json { head :ok }
      else
    		@supplier_account = @supplier.supplier_account
		    @service = Service.find params[:service_id]
